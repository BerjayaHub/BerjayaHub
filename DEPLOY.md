...
| 0134 | `0134_bongkar_bahan.sql` | **Bongkar bahan setengah jadi kembali jadi bahan bakunya.** UDANG PACK dibongkar → stok pack berkurang, udangnya kembali. Menambah tabel `bongkar_runs` + `bongkar_items`, RPC `bongkar_bahan(...)`, `batalkan_bongkar(uuid, text)`, dan `porsi_bongkar(uuid, numeric)`. Dua jenis `movement_type` baru: **`bongkar_out`** & **`bongkar_in`** (batasan `stock_movements` diganti). ⚠️ **Batas atas per bahan ditegakkan server** — 5 pack yang resepnya 50gr tidak bisa menghasilkan 500gr; tanpa itu fitur ini adalah alat mencetak stok dari udara. Bahan yang kembali **tidak membawa `unit_cost`** (aturan 0118/0123: hanya pembelian yang mengisinya), jadi rata-rata biaya bahan baku tidak tergoyang. Resep **wajib ada** — ia satu-satunya yang menyatakan isi sebuah pack. Ini kejadian BARU, bukan `hapus_produksi`: packnya boleh datang dari kiriman CK. Stok boleh menembus nol (konsisten dengan produksi 0020); layar yang memperingatkan. ℹ️ Tidak mengubah data lama |
| 0133 | `0133_kiriman_salah_alamat.sql` | **Kiriman salah alamat punya jalan keluar.** Status `cancelled` sudah ada di batasan `dispatches` sejak `0022` tapi **tidak pernah ada fungsi yang mengisinya** — kiriman salah alamat wajib diterima outlet yang salah. Dua RPC baru: `batalkan_kiriman(uuid, text)` untuk yang **belum diterima** (stok belum bergerak; boleh oleh **pengirim maupun penerima**; alasan wajib; kiriman lama pra-`0103` yang stoknya sudah terpotong saat dibuat dikembalikan lewat pergerakan penyeimbang), dan `teruskan_kiriman(uuid, uuid, text)` untuk yang **sudah diterima** — membuat **draft** surat jalan baru dari outlet yang salah ke tujuan yang benar, sebanyak yang **benar-benar diterima**. Menambah `dispatches.dibatalkan_at/_by`, `alasan_batal`, dan `koreksi_dari` (menyambungkan kiriman koreksi ke kiriman yang salah). Kiriman yang sudah diekspor ke ESB ditahan. ℹ️ Tidak mengubah data lama |
| 0132 | `0132_kiriman_nol_dan_keterangan.sql` | **Barang yang dipesan tapi TIDAK dikirim tetap tercatat, dan tiap baris punya keterangan.** Menambah `dispatch_items.keterangan` & `dispatch_items.ordered_qty`. `buat_draft_kiriman` dan `ubah_draft_kiriman` berhenti membuang baris ber-qty nol (`v_qty <= 0` → `v_qty < 0`), sehingga "diminta 10, dikirim 0" muncul di surat jalan alih-alih lenyap seolah tidak pernah dipesan. Batasan lama `sent_qty > 0` dicabut dan diganti `sent_qty >= 0` — nol punya arti, minus tidak. Menyunting draft **tidak** menghapus keterangan/ordered_qty yang tidak dikirim ulang klien (PWA lama tidak mengenal keduanya). RPC baru `lengkapi_keterangan_kiriman(uuid, jsonb)`: outlet boleh mengisi keterangan yang **masih kosong** saat menerima, tapi tidak pernah menimpa keterangan CK. Draft yang **seluruh** barisnya nol ditolak — arahkan ke "Tolak Order" beserta alasannya. ℹ️ Tidak mengubah data lama |
| 0131 | `0131_nota_batal_dan_koreksi.sql` | **Salah input nota bisa dibetulkan.** Dua aksi baru: `batalkan_nota(uuid, text)` — stok ditarik lewat pergerakan penyeimbang, kas dikembalikan kalau sudah dibayar, **alasan wajib**, notanya TIDAK dihapus (status `dibatalkan`); dan `koreksi_nota(...)` — memperbaiki tanggal/bahan/harga **termasuk pada nota yang sudah lunas**, dengan kas disesuaikan otomatis sebesar selisih totalnya lewat entri `cash_entries.penyesuaian_nota`. Trigger `trg_tolak_ubah_nota_lunas` (0122) diganti `trg_jaga_ubah_item_nota`: nota lunas kini bisa diubah **hanya lewat `koreksi_nota`** (penanda sesi `berjaya.koreksi_nota`), sementara jalur langsung tetap tertutup supaya PWA lama tidak bisa mengubah nilai tanpa kas ikut bergerak. Nota batal tidak bisa dibayar, diubah, atau diekspor ke ESB; nota yang **sudah** diekspor ESB ditahan sampai tanda ekspornya dibatalkan. `payment_status` bertambah nilai `batal`. ⚠️ Tidak mengubah data lama — semua nota yang ada tetap berstatus `aktif` |
| 0130 | `0130_bucket_foto_nota.sql` | **Perbaikan: foto nota tidak bisa diunggah, "Bucket not found".** Bucket `receipt-photos` memang tidak pernah lahir. Sebabnya bentuk `0084`: empat `create policy` **tanpa** `drop policy if exists` di baris 71–83 mendahului pembuatan bucket di baris 263, jadi begitu `0084` dijalankan ulang ia berhenti di baris 71 dan tidak pernah sampai ke sana. Tabel notanya ada, layarnya ada, nota bisa disimpan — hanya tombol unggahnya yang mati. 0130 memastikan **seluruh** bucket ada (`on conflict do nothing`, jadi yang sudah benar tidak berubah sifatnya), memasang ulang tiga kebijakan `receipt_photo_*`, dan **melaporkan hasilnya** lewat `raise notice`. Aman dijalankan berulang. ℹ️ Tidak mengubah data apa pun |
| 0129 | `0129_satu_bahan_satu_baris.sql` | **Satu bahan tidak boleh muncul dua baris** dalam satu order, surat jalan, atau nota. Unique index pada `stock_order_items(order_id, product_id)`, `dispatch_items(dispatch_id, product_id)`, dan `goods_receipt_items(receipt_id, product_id)`, plus trigger `tolak_bahan_kembar_*` yang pesannya **menyebut nama bahannya** — tanpa itu yang muncul di layar staff adalah "duplicate key value violates unique constraint", kalimat yang tidak bisa ditindaklanjuti siapa pun. ⚠️ **MENGUBAH DATA LAMA**: baris kembar yang sudah tersimpan **dijumlahkan** jadi satu (100 + 150 → 250). Total qty-nya tidak berubah; yang berubah jumlah barisnya. Satu pengecualian yang disengaja: kalau salah satu baris nota **belum berharga**, harga gabungannya **dikosongkan** (bukan diambil dari baris yang berharga) — nota itu perlu diisi ulang harganya sebelum bisa dilunasi. Penggabungannya mematikan `trg_tolak_ubah_nota_lunas` (0122) dan `trg_tolak_ubah_kiriman_terekspor` (0128) untuk sesaat, lalu **menyalakannya lagi**. `stock_movements.unit_cost` ikut disamakan supaya laporan biaya bahan tidak memakai angka lama |
| 0128 | `0128_ekspor_esb_transfer.sql` | **Ekspor kiriman ke template ESB "Simple Transfer".** Lanjutan `0127`, bentuknya sama persis: `dispatches.esb_exported_at` / `esb_exported_by`, RPC `tandai_kiriman_esb` (dipanggil SESUDAH berkasnya jadi) dan `batalkan_tanda_kiriman_esb`. **Tidak ada tabel baru** — `esb_master` & `esb_map` dari `0127` dipakai apa adanya, karena Origin/Destination sumbernya sama-sama nama outlet. `tandai_kiriman_esb` hanya menerima kiriman berstatus **`received`** dan yang punya **`received_at`**: barang yang masih di jalan belum boleh menambah stok di ESB, dan sel `Date` yang kosong diisi ESB dengan tanggal unggah. Plus trigger `trg_tolak_ubah_kiriman_terekspor` yang mengunci `dispatch_items` milik kiriman yang sudah bertanda. ⚠️ **Sesudah ini, mengoreksi qty terima kiriman yang sudah diekspor akan ditolak** — batalkan tandanya dulu, perbaiki, lalu unggah ulang ke ESB (berkas lamanya dihapus di sana) |
