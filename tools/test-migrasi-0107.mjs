/**
 * MIGRATION 0107 (kabar shift dari penilaian ulang presensi) DI POSTGRES SUNGGUHAN.
 *
...
// 0104 -> 0105 -> 0107, persis urutan di produksi. Masing-masing menimpa yang
// sebelumnya, jadi yang diuji selalu versi TERAKHIR.
...
console.log('  0104 -> 0105 -> 0107 terpasang.');
await jalankan('0107_kabar_shift_dari_penilaian_ulang.sql');
...
// =====================================================================
// 3b. PENILAIAN ULANG PRESENSI IKUT JADI KABAR SHIFT (0107)
//
// Skenario yang dilaporkan: jadwal Rabu dikoreksi hari Jumat, lalu admin
// menilai ulang presensinya. Angka keterlambatan orangnya berubah — dan tanpa
// tanda apa pun, ia baru tahu saat tunjangannya sudah dihitung.
// =====================================================================
await q(`delete from shift_schedules where user_id = $1`, [AKU]);
await q(`delete from attendance_records where user_id = $1`, [AKU]);

let kabarShift = (await lencana(BU, SHIFTAN)).modul.shift.terakhir_aktivitas;
cek('3b. tanpa jadwal & tanpa penilaian ulang: belum ada kabar', kabarShift, null);

// Presensi biasa (belum dinilai ulang) BUKAN kabar. Kalau ia ikut dihitung,
// kartu Shift bertitik setiap kali orangnya clock in — dan titik yang muncul
// tiap hari berhenti menyampaikan apa pun.
await q(`insert into attendance_records (user_id, business_unit_id, outlet_id) values ($1,$2,$3)`, [AKU, BU, SHIFTAN]);
cek('3b. clock in biasa BUKAN kabar', (await lencana(BU, SHIFTAN)).modul.shift.terakhir_aktivitas, null);

// Dinilai ulang -> baru jadi kabar.
await q(`update attendance_records set late_dinilai_ulang_at = now() where user_id = $1`, [AKU]);
kabarShift = (await lencana(BU, SHIFTAN)).modul.shift.terakhir_aktivitas;
benar('3b. sesudah dinilai ulang: ADA kabar', kabarShift != null);

// Penilaian ulang presensi ORANG LAIN bukan kabar untukku.
await q(`insert into attendance_records (user_id, business_unit_id, outlet_id, late_dinilai_ulang_at)
  values ($1,$2,$3, now() + interval '2 hours')`, [REKAN, BU, SHIFTAN]);
const kabarSesudahRekan = (await lencana(BU, SHIFTAN)).modul.shift.terakhir_aktivitas;
cek('3b. penilaian ulang rekan tidak menggeser kabarku', kabarSesudahRekan, kabarShift);

// Yang paling BARU di antara dua sumber yang menang.
await q(`insert into shift_schedules (business_unit_id, outlet_id, user_id, work_date, shift_id)
  values ($1,$2,$3,${HARI_INI} + 5,$4)`, [BU, SHIFTAN, AKU, PAGI]);
await q(`update shift_schedules set updated_at = now() + interval '1 hour' where user_id = $1`, [AKU]);
const kabarTerbaru = (await lencana(BU, SHIFTAN)).modul.shift.terakhir_aktivitas;
benar('3b. jadwal yang lebih baru menang atas penilaian ulang',
  Date.parse(kabarTerbaru) > Date.parse(kabarShift), `${kabarTerbaru} vs ${kabarShift}`);

await q(`delete from attendance_records where user_id in ($1,$2)`, [AKU, REKAN]);
await q(`delete from shift_schedules where user_id = $1`, [AKU]);
...
await db.close();
console.log(gagal === 0 ? '✅ migrasi 0107: semua lulus' : `❌ migrasi 0107: ${gagal} gagal`);
